...
不过，分工协作并非免费的午餐——什么时候**不该**用多智能体（Cognition 与 Anthropic 的著名路线之争，以及"读写分离"决策判据），见**附录 A.5**。
...
> **📎 配套附录**
>
> - **附录 A.6**：可运行的 Orchestrator-Worker 系统实现（`sampleCode/agent_system`，依赖分波调度 + 异步并发 + 实测耗时数据）
...
> - **附录 A.6**：HITLGate 的最小实现——「能力等级传递」原则在 sampleCode 里如何落地
...
- 可运行实现：`sampleCode/agent_system` 的 Orchestrator 用显式 `AgentMessage(sender, receiver, content)` 投递 Worker 间消息（见附录 A.6），与下面方案二的共享状态范式形成对照
...
> - **可运行对照实现**：`sampleCode/agent_system/frameworks/`——把附录 A.6 的同一个 Orchestrator-Worker 场景分别用 LangGraph 和 CrewAI 重写一遍，用真实模型实测验证了上表的几条评分：CrewAI 的"HITL 支持 ⭐⭐ 有限"和"并发支持 ⭐⭐ 有限"不是纸面判断——CrewAI 没有工具调用级的中断机制（HITL 只能做到任务粒度），跨 Crew 并行也得自己在外层手写 `asyncio.gather`（框架不像 LangGraph 那样能从图结构自动识别独立分支）
...
### A.5 单 Agent vs 多 Agent：一场必须了解的路线之争

> 关联正文：**Part 1 → 1.2 分工协作的价值**

本讲正文系统讲解了多智能体架构，但有一个前置问题值得专门回答：**什么时候根本就不该用多智能体？** 2025 年 6 月，两篇几乎同时发布、结论看似相反的文章把这个问题变成了行业级辩论，也是面试高频题。

#### A.5.1 反方：Cognition《Don't Build Multi-Agents》

Devin 团队（Cognition）的核心论点建立在两条原则上：

1. **共享完整上下文（Share full context）**：每个 Agent 的决策质量取决于它看到的信息。并行的 Subagent 各自只看到任务的一个切片，无法看到彼此的工作。
2. **行动携带隐含决策（Actions carry implicit decisions）**：Agent 的每个动作都隐含着对模糊问题的解释。两个看不到彼此的 Agent 会做出**相互冲突的隐含决策**。

经典反例：让两个 Subagent 分别实现一个游戏的两个部分，一个做出了写实风格，另一个做出了卡通风格——各自局部正确，拼起来是灾难。**冲突不在代码层面（可以合并），而在决策层面（无法合并）**。

Cognition 的结论：写任务（代码生成）应该用**单线程 Agent + 上下文压缩**来扩展任务长度，而不是并行分工。

#### A.5.2 正方：Anthropic《How we built our multi-agent research system》

同月，Anthropic 公布其 Research 功能的多智能体架构（Orchestrator + 并行检索 Worker），内部评测显示多智能体相比单 Agent **性能提升约 90%**，代价是 **token 消耗约 15 倍**。

它为什么成功了？看任务性质：

- **读密集（read-heavy）**：Worker 们并行搜索、阅读不同来源——读操作之间**没有相互冲突的隐含决策**（读 A 文档不影响 B 文档的内容）；
- **结果天然可合并**：每个 Worker 返回的是压缩后的事实摘要，Orchestrator 做信息综合，而不是拼接各 Worker 的"作品"；
- **超出单上下文容量**：要读的材料总量远超一个窗口，隔离（每个 Worker 独立窗口）是物理必需（见 Harness 工程与上下文工程专题的"隔离"一节）。

#### A.5.3 综合：争论双方其实没有分歧

把两篇文章放在一起看，它们描述的是同一条规律的两端：

> **决策判据 = 子任务之间的耦合度。**
> 子任务的隐含决策互相影响（写代码、写文章）→ 强耦合 → 单 Agent 串行，用压缩/卸载扩展长度；
> 子任务只是各自收集信息（搜索、调研、多文件排查）→ 弱耦合 → 多 Agent 并行，用隔离扩展容量。
...
任务能拆成子任务吗？
 ├── 不能 → 单 Agent
 └── 能 → 子任务之间共享"待决定的设计选择"吗？
      ├── 是（写密集：代码/文档/方案） → 单 Agent + 上下文工程
      │     （并行的代价是决策冲突，得不偿失）
      └── 否（读密集：检索/调研/排查） → Orchestrator-Worker 并行
            （前提：预算接受 ~15x token 成本）
```

两个推论：

1. **"读写分离"是多智能体设计的第一原则**：即使在写密集任务里，也可以把其中的读密集环节（如"调研这个库的用法"）剥出来并行——Claude Code 的 Subagent 主要用于搜索/探索类任务而非并行写代码，正是这个原则的体现（见 4.1 节）。
2. **模型变强会移动这条边界**：Cognition 文章自己也承认，随着模型长上下文与跨 Agent 沟通能力提升，写任务的并行协作未来可能可行——把这条边界看作随模型能力移动的动态线，而不是教条。

### A.6 可运行的 Orchestrator-Worker 系统实现——从架构图到能跑的代码

> 关联正文：**Part 2.1（Orchestrator-Worker 架构）/ Part 2.2（Supervisor 模式与 HITL）/ Part 2.3（A2A 通信协议）**

Part 2.1-2.3 讲的架构模式、审批机制、通信协议，在 `sampleCode/agent_system/` 里有一份完整的可运行实现（`core/orchestrator.py` + `core/hitl.py`），延续 Week 9 附录 A.2 的单 Agent 地基。任务场景不变：还是"改代码通过 pytest"，这次同时派发给两个相互独立的子任务（`moving_average` / `flatten_list`），各自先由 Coder 实现，再由 Reviewer 独立复核。

**与附录 A.2（LLM 动态规划）的关系**：A.2 演示的是让 Orchestrator LLM 自己生成子任务 JSON；这里的实现反过来——子任务图是硬编码的（见 `demo_w10.py`），换取任务场景固定、离线可复现（呼应 Week 9 mock 后端"先写死剧本，再换真模型"的教学取舍）。两者结论互补：动态规划解决"怎么分解任务"，这份实现解决"分解好之后怎么调度、怎么审批、怎么通信"。

**1. 依赖分波调度（Part 2.4 依赖图规划的最简单落地方式）**

不需要完整的拓扑排序算法，`Orchestrator.run()` 每轮只做一件事：找出"依赖已全部完成"的子任务集合作为一波（wave），并发跑完这一波，再找下一波：
...
本例的依赖图很简单——两个 Coder 子任务无依赖（wave 1 并发），两个 Reviewer 各自依赖同任务的 Coder（wave 2 并发）。真实系统的依赖图可能更复杂（钻石依赖、多对多），但"反复找 ready 集合"这个思路本身可以直接推广。

**2. 真异步并发 + 实测耗时数据**

`ReActAgent.arun()` 是 `run()` 的异步版本：LLM 决策改用 `await self.backend.adecide(...)`，工具执行（可能是阻塞的 pytest 子进程）丢进线程池 `asyncio.to_thread`。`OpenAICompatBackend.adecide` 用 `AsyncOpenAI` 发起真正的非阻塞网络请求——这一点很关键：如果只是把同步调用包一层 `asyncio.to_thread`（`LLMBackend.adecide` 的默认兜底做法），多个 Worker 的网络等待时间不会重叠，只是排队占用线程池，达不到真正的并发收益。

实测数据（`--backend lkeap`，两个独立任务，改变并发上限对比）：

| 并发上限 | wave 1（2 个 Coder） | wave 2（2 个 Reviewer） | 总耗时 |
|---|---|---|---|
| 4（默认，并行） | 11.7s | 13.7s | 25.4s |
| 1（强制串行） | 22.4s | 23.5s | 45.9s |

约 1.8 倍加速——具体数值会随网络状况波动，但方向性结论稳定：I/O 密集型的 LLM 调用，并发数越接近独立子任务数，收益越接近线性（受限于外部 API 的速率限制和网络带宽）。

**3. HITL 审批：能力等级传递的最小实现**

对应 Part 2.2"解法三"——`run_pytest`（代码执行）首次调用即被视为最高风险，批准一次后本次任务内自动放行；`write_file` 每次都要重新批准，因为每次写入的内容不同：
...
class HITLGate:
    def check(self, tool_name, tool_args):
        ...
        if tool_name not in self.require_every_time and tool_name in self._approved_once:
            return None          # 已批准过的能力，本次任务内直接放行
        approved = self.approve(tool_name, tool_args)
        ...
```

`demo_w10.py` 默认用 `auto_approve_with_log` 自动批准但打印日志，让审批点可见；加 `--interactive` 改成真正阻塞在 `input()` 等人工确认——这时候审批检查本身也要丢进 `asyncio.to_thread`，否则一个 Worker 卡在等输入会冻结整个事件循环里的其他并发 Worker（这是实现时踩过的一个真实坑）。

**4. A2A 消息传递 vs 附录 A.2 的共享状态**

Reviewer 不会读 Coder 的工作记忆，Orchestrator 把 Coder 的 `final_answer` 包成一条显式 `AgentMessage(sender, receiver, content)` 投递给依赖它的下游子任务：

```python
inbox[other.id].append(AgentMessage(
    sender=subtask.id, receiver=other.id,
    content=result.final_answer or "(无最终答复)",
))
```

这是 Part 2.3 的**方案一（消息传递）**；附录 A.2 的 `results[dep]` 字典读写是**方案二（共享状态）**——两种范式在这里刻意做出不同选择，便于对照：消息传递的耦合更松（Reviewer 只能看到 Coder"想让它看到"的结论，看不到 Coder 的完整工作记忆），代价是信息传递更"窄"，Reviewer 如果需要更多上下文就得靠 Coder 把它写进 `final_answer`。

**5. 与附录 A.5"读写分离"原则的呼应**

这份实现的并行是安全的，原因恰好印证附录 A.5 的判据：两个 Coder 子任务处理的是**完全独立**的文件（`math_utils.py` / `list_utils.py`），互相之间没有共享的"待决定设计选择"——不存在 A.5 里"一个写实风格、一个卡通风格"式的隐含决策冲突。而同一任务内 Coder→Reviewer 之间是真实的强依赖（Reviewer 要读 Coder 写完的文件），所以被安排在不同的 wave 里串行，而不是并行——这正是 Orchestrator 依赖调度存在的意义：自动识别"哪些子任务解耦、可以并行"与"哪些子任务耦合、必须等待"。

**已知局限**：本实现暂未包含 Worker 失败重试/降级逻辑（附录 A.3 讨论的三种策略）——Worker 抛异常会通过 `asyncio.gather` 直接向上传播、中断整个 `Orchestrator.run()`。这是有意暂缓的工程债，留到 Week 11 评估 harness 阶段一并处理（那时候需要区分"Worker 失败"与"评估不通过"两种不同信号）。

**运行方式：**

```bash
cd sampleCode/agent_system
python demo_w10.py                                        # 默认离线 mock 后端
python demo_w10.py --backend lkeap                         # 真实大模型，观察真实并发耗时
python demo_w10.py --backend lkeap --max-concurrency 1     # 强制串行，对比耗时
python demo_w10.py --backend lkeap --interactive           # HITL 改为真正等待终端确认
```
...
| 文件 | 对应讲义章节 |
|------|------|
| `core/orchestrator.py` | Part 2.1（Orchestrator-Worker）、Part 2.3（A2A 通信协议）、Part 2.4（依赖图规划） |
| `core/hitl.py` | Part 2.2（Supervisor 模式与 HITL） |
| `core/agent.py`（`arun`） | Part 2.1（异步 Worker 执行） |
| `core/llm_backend.py`（`adecide`） | Part 2.1（真异步网络调用） |

完整代码见 `sampleCode/agent_system/README.md`。这套系统会在 Week 11（MCP 工具生态 + 评估 harness）继续演进。

**与框架版本的对照**：同一个场景另外用 LangGraph、CrewAI 各实现了一遍，见 `sampleCode/agent_system/frameworks/`（对应 Part 3.4 的配套附录说明），可以直接对比手搓版和两个主流框架在依赖调度、A2A 通信、HITL 粒度上的具体差异。
