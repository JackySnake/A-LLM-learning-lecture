...
> - **附录 A.5**：可运行的 MCP 封装（`sampleCode/agent_system`，把 Week 9 的工具真正接成 stdio server）
...
> **📎 配套附录**
>
> - **附录 A.5**：`ToolRegistry.scoped()`（原则一/二的硬约束实现）+ `AuditLog`（原则三的审计轨迹落盘）
...
> **📎 配套附录**
>
> - **附录 A.5**：`core/eval.py` 的可运行实现——`evaluate_outcome()` 独立重跑 pytest、`evaluate_trajectory()` 的三条启发式规则，含一次真实观察到的"Agent 自述 PASS 但其实没验证"案例
...
### A.5 可运行的 MCP 封装 + 评估 harness（`sampleCode/agent_system`）

> 关联正文：**Part 1.7（MCP 实际接入）/ Part 2.3（Trust by Design）/ Part 3.2（Outcome vs Trajectory）**

延续 Week 9 附录 A.2、Week 10 附录 A.6 的同一套 sampleCode，Phase 5 收官这一周补齐最后一块：
把 Week 9 就有的 4 个工具（`list_dir`/`read_file`/`write_file`/`run_pytest`）真正封装成 MCP
Server，并给"改代码通过 pytest"这个固定任务场景配上一份可运行的评估 harness。

**1. MCP Server + Client（对应 Part 1.7、附录 A.1）**

`mcp_server.py` 用官方 mcp SDK 的底层 `Server` API（和附录 A.1 的写法一致）把 4 个工具原样
暴露成 stdio server；`core/mcp_backend.py` 的 `MCPToolRegistry` 反过来在 Agent 侧接住，
实现和 `core/tools.py` 的 `ToolRegistry` 完全相同的接口（`specs()`/`describe_for_prompt()`/
`execute()`），唯一的差别是 `execute()` 变成了协程——因为工具调用现在要经过一次真实的
跨进程 IPC，而不是进程内函数调用：
...
`ReActAgent` 完全不需要为此改一行代码——Week 10 给 `arun()` 加的
`inspect.iscoroutinefunction(self.tools.execute)` 判断，当时是为了兼容 Week 9 的同步
`ToolRegistry`，现在正好也接住了这个异步版本。这是本项目第三次验证"接口稳定、实现可替换"：
第一次是 memory.py 的向量库可替换，第二次是 frameworks/ 的编排层可替换，这次是工具执行的
传输协议可替换。

**2. 白名单升级：从软约束到硬约束（对应 Part 2.3 原则一、原则二）**

MCP 协议本身就是一层更强的白名单边界——客户端只能调用 `list_tools()` 声明过的工具名，
这比 Week 9 `ToolRegistry.execute()` 的字典查找又多了一层进程边界。`core/tools.py` 同时
补了 `ToolRegistry.scoped(names)`，把"最小权限"从 prompt 里的文字约束升级成注册表层面的
硬约束——`demo_w10.py` 的 Reviewer worker 现在用 `registry.scoped([...])` 拿掉了
`write_file`，它的 LLM 调用里根本不存在这个工具，不是"选择不用"。

**3. 审计日志（对应 Part 2.3 原则三）**

`core/audit.py` 的 `AuditLog` 把每一步的完整记录（时间戳、思考、工具、参数、结果、是否
经过 HITL 批准）落盘成 JSONL，接入 `ReActAgent` 的可选 `audit` 参数。这条日志也是诊断
Trajectory Eval 问题的原始数据来源。

**4. 评估 harness：Outcome + Trajectory（对应 Part 3.2、附录 A.3）**

`core/eval.py` 分别实现两种评估范式：

```python
def evaluate_outcome(sandbox: pathlib.Path) -> bool:
    """不信任 Agent 的自我报告，独立重新跑一次 pytest——呼应附录 A.3
    FAIL_TO_PASS/PASS_TO_PASS 测试在评测时独立验证的做法。"""
    kit = CodingToolkit(sandbox)
    observation = kit.run_pytest()
    return "passed" in observation and "failed" not in observation
```

这一条"不信任自述"不是理论上的谨慎——`sampleCode/agent_system/frameworks/README.md`
记录了一次真实观察：LangGraph/CrewAI 版本第一次实测时，Reviewer 曾经读完代码就直接下
PASS 结论，没有真的调用 `run_pytest` 独立验证。如果 Outcome Eval 直接解析 Agent 的
`final_answer`，这类情况会被误判为成功；独立重跑 pytest 才是唯一可靠的判据。

`evaluate_trajectory()` 是基于规则的启发式 Trajectory Eval（不需要人工标注参考轨迹），
检查"先探索后写""验证再回答""有没有连续重复调用"三类最常见的过程问题——这是 Part 3.2
说的"标注成本高"问题的一个简化解法，抓大放小，不是全面的轨迹质量评分。

`demo_w11.py` 对 3 个独立任务（`moving_average`/`flatten_list`/`is_palindrome`）各自跑一遍
MCP 封装的 Agent，汇总成 Resolve Rate 报告：

```
Resolve Rate：3/3（100%）
  [moving_average] ✅ 通过 | 7 步 | 轨迹：无异常
  [flatten_list]   ✅ 通过 | 7 步 | 轨迹：无异常
  [is_palindrome]  ✅ 通过 | 7 步 | 轨迹：无异常
```
...
python demo_w11.py                 # 默认离线 mock 后端
python demo_w11.py --backend lkeap # 真实大模型
```
...
| 文件 | 对应讲义章节 |
|------|------|
| `mcp_server.py` / `core/mcp_backend.py` | Part 1.7（MCP 实际接入）、附录 A.1 |
| `core/tools.py`（`scoped()`） | Part 2.3（Trust by Design 原则一/二） |
| `core/audit.py` | Part 2.3（Trust by Design 原则三：审计轨迹） |
| `core/eval.py` | Part 3.2（Outcome vs Trajectory）、附录 A.3（SWE-Bench 评测流程） |

完整代码见 `sampleCode/agent_system/README.md`。这套系统从 Week 9 到 Week 11 完整走了一遍
"单 Agent → 多 Agent → 生态接入 + 评估"的演进路径，Phase 5 到此收官。
