...
> **📎 配套附录**
>
> - **附录 A.2**：可运行的 ReAct Agent 完整实现（`sampleCode/agent_system`，含工具注册、多轮协议、错误处理，离线可跑通）
...
### A.2 ReAct Agent 完整实现——从教学片段到可运行系统

> 关联正文：**Part 2 & Part 3 → ReAct 执行流程 + 工具调用生命周期**

Part 2.3 展示的 `思考：/行动：/观察：` 文本格式，是 ReAct 论文原始设计、也是**阶段一"Prompt 工程驱动"**（3.1 节）的实现方式——靠正则解析模型输出，格式不稳定，工具名拼错、参数格式错误都会让整个循环崩溃。3.2-3.3 节讲的现代 Function Calling 范式（结构化 JSON + JSON Schema 约束）正是为了解决这个问题而生的。

本节不再给一段孤立的教学代码片段，而是直接指向一个**可运行的完整实现**：`sampleCode/agent_system/`。它用现代 Function Calling 协议实现了同样的"思考→行动→观察"循环，并且额外解决了教学片段通常会忽略的三个工程问题：

1. **多轮协议的 id 配对**：每次工具调用都带唯一 `tool_call_id`，工具结果必须用同一个 id 回指——这是 Anthropic/OpenAI 多轮 tool 协议的强制要求，随手写的教学代码很容易漏掉。
2. **厂商中性的事件流**：Agent 循环本身不关心是 Claude 还是 GPT 在做决策，`LLMBackend` 接口统一收发一份中性事件流，各后端负责把它翻译成自己的原生格式（见 `core/llm_backend.py`）。
3. **离线可跑通**：默认用一个规则驱动的 `MockCodingBackend` 演示完整循环（含"第一版实现失败→读报错→修复→通过"的真实反馈闭环），不需要 API Key 就能看到 ReAct 的核心机制；换上真实模型（Claude / 腾讯云 lkeap 等 OpenAI 兼容端点）只需切换 `--backend` 参数，循环代码不变。

**核心循环**（节选自 `core/agent.py`，完整逻辑约 150 行）：

```python
# Reason → Act → Observe 的主循环骨架
for step_idx in range(1, self.max_steps + 1):
    action = self.backend.decide(
        system_prompt=system_prompt,
        messages=self.memory.working.view(),   # 中性事件流，非纯文本拼接
        tools=self.tools.specs(),               # 厂商中性的工具规格
    )

    if action.is_final:                         # 模型给出最终答案 → 结束
    ...
    self.memory.working.add("assistant", thought=action.thought,
                             tool_call={"id": call_id, "name": action.tool_name,
                                        "args": action.tool_args})

    observation = self.tools.execute(action.tool_name, action.tool_args)  # Act
    self.memory.working.add("tool", observation, tool_call_id=call_id)    # Observe
```
...
python demo_w9.py                  # 默认离线 mock 后端，无需 API Key
python demo_w9.py --backend lkeap  # 切换到真实大模型（需配置 .env，见 README）
```

任务是"实现 `moving_average` 函数并通过 pytest"——观察输出会看到 Agent 第一版实现让测试失败，读到报错后定位边界条件问题并修复，这正是 ReAct 区别于一次性 CoT 的地方：**行动的真实结果会反过来修正后续推理**，而不是模型一次性把所有步骤想完。

**代码结构与讲义章节的对应关系：**

| 文件 | 对应讲义章节 |
|------|------|
| `core/llm_backend.py` | Part 3.2-3.3（Function Calling 协议、JSON Schema） |
| `core/tools.py` | Part 3.3、3.6（工具注册、可靠性工程） |
| `core/memory.py` | Part 4.2（四层记忆架构） |
| `core/agent.py` | Part 2.2-2.4（ReAct 循环、停止条件、最大步数） |

完整代码见 `sampleCode/agent_system/README.md`（含架构说明、每周演进计划）。这套系统会在 Week 10（多智能体协作）、Week 11（MCP 工具生态 + 评估）持续演进，用同一个"改代码通过测试"的编程助手场景，逐周长出新能力。
...
**与本项目 sampleCode 的关系**：`sampleCode/agent_system/core/memory.py` 里的 `SemanticMemory`/`EpisodicMemory` 为了保持零外部依赖、离线可跑通，检索用的是关键词重叠打分（`_overlap_score`），而不是本节的向量余弦相似度——但对外暴露的 `store()` / `retrieve()` 接口与真实向量库版本完全一致。这是刻意的工程取舍：**接口先稳定，实现可替换**——把 `_overlap_score` 换成本节的 Chroma + Embedding 方案，Agent 循环的其余部分不需要任何改动。
