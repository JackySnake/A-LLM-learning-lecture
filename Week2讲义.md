...
> **All-Gather vs Reduce-Scatter 的区别**
>
> - **All-Gather（参数拉取）**：把所有卡各自持有的参数碎片"广播拼合"，每卡得到完整副本。信息方向：分散 → 每卡各得完整版。
> - **Reduce-Scatter（梯度汇聚）**：把所有卡各自算出的梯度先求和（Reduce），再按分片分发给各卡（Scatter），每卡只拿到属于自己负责的那部分。信息方向：所有卡 → 聚合后按职责分发。
