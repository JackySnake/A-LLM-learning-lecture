...
- **AIME**（American Invitational Mathematics Examination，美国数学邀请赛）：这不是专门为 LLM 设计的基准，而是**真实存在的高中数学竞赛**（面向 AMC 竞赛的高分选手），社区直接把每年的真实赛题拿来评测模型。由于比赛**每年出新题**，天然具有类似 LiveCodeBench 的"时间切片抗污染"效果——只要用"模型发布日期之后"举行的那一年比赛题，就能保证模型没在训练数据中见过。2025 年的 AIME 赛题被广泛用作评测 o1/o3/DeepSeek-R1 等推理模型的核心基准之一（"每年出新题"设计的三个代价——跨年可比性、题量统计噪声、饱和加速——详见**附录 A.6**）。
...
**ARC-AGI-3**（ARC Prize Foundation，2026-03-25 发布，预览版 2025-07，论文 arXiv:2603.24621）把这一逻辑推到了更根本的层面：它是系列中第一个**交互式推理基准**（Interactive Reasoning Benchmark）。评测对象面对的不再是静态网格谜题，而是一个**回合制的抽象小游戏环境**——agent 被放入一个从未见过的环境中，**不被告知任何任务目标**，必须依靠试探性动作自行探索环境动力学、推断"这一关的目标是什么"、建立内部的世界模型，并据此规划动作序列。评分方式也同步从"通过/不通过"改为 **RHAE（Relative Human Action Efficiency，相对人类动作效率）**：不仅要求通关，还以人类完成同一环境所用的动作数为基准衡量效率，把"高效探索"正式纳入智能的度量。

ARC-AGI-3 发布时产生了 2026 年评测领域最具冲击力的结果：**人类基线约 100%，而所有前沿模型得分全部低于 1%**——Gemini 3.1 Pro 0.37%、GPT-5.4 0.26%、Claude Opus 4.6 0.25%、Grok-4.20 0.00%（均为一手来源确认的数字）。更耐人寻味的是：30 天开发者预览中，一套简单的 CNN 表征 + 图搜索的专门方法即达到 12.58%，数倍于所有通用大模型——这强烈提示通用 LLM 在"交互式探索 + 在线目标推断"上的缺口是**结构性**的，而非单纯的规模问题。从评测演进的角度看，ARC-AGI-3 是"难度升级"逻辑的极端形态：一次发布直接把区分度从"濒临饱和"重置到"接近零点"，同时把评测范式从"静态问答"整体推进到"agentic 交互"——它考察的能力栈（探索、目标推断、世界模型、序列规划）与 Week 11 讨论的 Agent 能力栈高度重合，是"能力评测"与"Agent 评测"开始合流的标志性事件。

一个对工程实践极具启发的后续案例：OpenAI 在发布后的官方复测中发现，同一模型（GPT-5.6）在官方默认 harness 下得分 13.3%，仅开启 **retained reasoning 与 compaction**（保留推理轨迹 + 上下文压缩，机制见专题《Harness工程与上下文工程》）两项 harness 配置后，得分即升至 38.3%——接近三倍的差距全部来自模型之外的"外骨骼"。这与 Part 1.4 "评分方式影响分数"是同一主题在 agentic 时代的升级版：**当评测对象从"裸模型"变成"模型 + harness"系统，分数就不再是模型固有的属性，而是整个系统的属性**——跨厂商比较 agentic 基准分数时，必须先问清楚"harness 是什么"。
...
| 通用推理 | BBH / DROP | → MuSR（程序化生成）→ ARC-AGI-2 → **ARC-AGI-3（交互式环境）** | 精确匹配 / 规则应用正确性 → RHAE（对标人类动作效率） | 程序化生成新题 / 提高组合推理门槛 / 交互式环境（目标与动力学皆不可记忆） |
...
LLM-as-Judge 系列方法虽然自动化程度高，但仍然是"用一个模型的判断代表人类偏好"——裁判模型自身的偏见会传导到评测结果中。**Chatbot Arena**（Zheng et al., 2023，UC Berkeley LMSYS）提出了一种更彻底的方案：**跳过 LLM 裁判，直接让海量真实用户来投票**。

**先交代它的"实体"身份**：Chatbot Arena 不是一个数据集或评测脚本，而是一个**公开网站**（arena.ai，前身为 lmarena.ai 与 chat.lmsys.org），任何人打开浏览器即可免费使用。它由 UC Berkeley LMSYS/Sky Lab 团队（Vicuna 模型的同一团队）于 2023 年发起，凭借"边当产品、边当评测"的双重身份迅速成为事实上的行业标准榜单：用户免费体验各大厂商的最新模型（包括大量需付费订阅的模型），其投票行为则被汇总进公开排行榜——这解释了它为何能长期维持足够大的真实流量。2025 年团队从伯克利独立公司化（种子轮融资 1 亿美元），**2026 年 1 月品牌正式更名为 Arena**，此时平台累计约 700 万次盲测投票、覆盖 360+ 模型。网站提供三种交互模式：**Battle**（系统随机抽取两个匿名模型对战，这是计入排行榜的投票来源）、**Side-by-Side**（用户自选两个具名模型对比）与 **Direct Chat**（与单一模型直接对话）——后两者是体验功能。

**投票如何变成排名**：在 Battle 模式中，用户输入任意提示词，两个匿名模型分别生成回答（身份隐藏以避免品牌偏见），用户投票选出更好的一个（或判平局）。海量盲测投票积累后，系统用类似国际象棋 Elo 评分系统的统计模型为每个模型计算一个可比较的分数并排名（Elo 到 Bradley-Terry 模型演进的数学原理见**附录 A.4**）。

**Arena 家族**：文本对战之外，平台已扩展为覆盖多种能力域的榜单家族——Vision Arena（多模态理解）、Text-to-Image Arena（文生图）、WebDev Arena（网站/前端开发）、Copilot Arena（代码补全）、Search Arena（联网搜索产品对比）等，盲测投票 + 统计排名的方法论在各域间复用。
...
（1）无法针对性优化的说法并不绝对——虽然没有固定题库，但模型仍然可能被训练成"讨好 Arena 场景下典型用户的答题风格"（如偏好使用 Markdown 列表、加粗、恰到好处的长度），这是一种更隐蔽的 Goodhart 化，Chatbot Arena 团队在 2024 年底为此专门推出了 **Style Control**（风格控制）机制——用统计模型剔除回答长度、Markdown 格式密度等风格因素的影响后重新计算排名，这个功能现已内置到官方榜单中，用户可以随时切换"原始排名"和"风格控制后排名"对比查看；

（2）众包评测需要持续的用户流量支撑才能保证统计显著性，运营成本不低；

（3）真实用户的偏好本身也有偏差——普通用户未必总能准确判断哪个答案在专业性、事实准确性上更好，Arena 分数更接近"大众好感度"而非"专家质量评估"，这也是为什么 Arena-Hard（3.2 节）要专门从 Arena 流量中筛出"区分度高的难题"作为补充，而不是完全替代 Arena 本身；

（4）**厂商信任问题**——排行榜"同版本同分"的前提（线上评测的模型 = 厂商公开发布的模型）依赖厂商按规则出牌：2025 年 4 月的 **Llama 4 事件**中，Meta 提交参评的 Maverick 版本被证实是一个"针对对话性特殊优化的实验版"，与其公开发布的权重表现差异明显，排行结果无法迁移到用户实际可用的模型上，引发社区对 Arena 公平性的激烈讨论；LMArena 官方随后公开回应该提交不符合平台期望，并更新了模型提交与审核政策。这说明众包榜单的公信力不仅由统计方法保障，同样依赖平台治理。
...
| 偏差类型 | 表现 | 典型缓解策略 |
|---------|------|-------------|
| **位置偏差（Position Bias）** | 成对比较时，裁判倾向于偏好排在特定位置（通常是先出现）的答案，与内容质量无关（概念、成因与验证实验见**附录 A.5**） | 双向评测：A-vs-B 和 B-vs-A 各跑一次，取平均结果，消除位置带来的系统性偏移 |
...
### 3.5 超越评测：LLM-as-Judge 作为训练基础设施

前面四节中的 LLM-as-Judge 都出现在**评测**语境——它是"考场里的评委"。但 LLM-as-Judge 在当今大模型技术栈里的影响远不止于此：它同时深度介入**训练**语境，扮演"训练场上的教练"。四组代表性用法：

**（1）偏好数据合成与 RLAIF**。Week 4 讲过 RLHF 依赖人类标注的偏好对训练奖励模型，但人工标注昂贵且难以规模化。Anthropic 的 **Constitutional AI**（2022）开创了 **RLAIF**（RL from AI Feedback）路线——用 LLM 依据一组显式原则（"宪法"）自行评判、修正和比较输出，以 AI 反馈替代人类反馈作为偏好信号。后续 DPO/IPO 类直接偏好优化算法使用的偏好对，也大量由强 LLM 标注产生——"judge 提供标签、策略习得偏好"已成为对齐数据生产的主流范式。

**（2）训练数据的质量过滤器**。指令数据合成（Self-Instruct 及其后继者）可以低成本产生海量样本，但质量参差。**AlpaGasus**（2023）等工作的思路是用强 LLM 为每条合成数据的质量打分，仅保留高分者参与训练——实验发现精筛后的小数据集效果可以超过全量数据。在这个角色里，LLM-as-Judge 充当的是**数据管线上的过滤阀**。

**（3）自蒸馏 / 自提升回路**。更进一步，judge 和生成者可以是**同一个模型**。Meta 的 **Self-Rewarding Language Models**（2024）让模型在每轮迭代中同时产出回答并为自己的回答打分，用高分样本进行下一轮 DPO 训练，实现无需外部裁判的迭代自提升（且在多轮迭代中，模型的 judge 能力本身也在同步改进）。类似思路也见于 Llama 系列的 **rejection sampling fine-tuning**：从模型的 N 个采样中用奖励模型/judge 选出最优回答，再蒸馏回模型自身——judge 在这里是"自蒸馏"回路的选优闸门。

**（4）RL 阶段的在线评委**。2025 年后推理模型训练的主流范式是 **RLVR**（Reinforcement Learning with Verifiable Rewards，见 Week 7）——可验证任务（数学答案算对、代码单测跑通）用规则化检验器提供奖励，客观且抗刷分。但对写作、开放问答、多轮指令遵循等**不可验证**任务，不存在可编程的规则检验器，主流做法是直接接入 LLM judge 作为奖励函数；**DeepSeek-GRM**（2025）这类"生成式奖励模型"进一步通过在推理时采样多份评分并投票来提高 judge 的可靠性。Agent 场景同理：任务完成与否往往没有程序化的 ground truth，LLM-as-Judge 是 Week 11 轨迹评估（Trajectory Eval）和 Agent RL（见专题《AgenticRL与Agent训练》）奖励信号的主力来源。

**双重身份带来的自指闭环**。当 judge 既提供训练信号、又充当评测工具时，两种身份叠加会产生一个结构性风险：模型在训练中向着 judge 的口味优化——3.4 节的四种偏差在训练侧被放大为 reward hacking 的具体形态（答案人为变长、格式更花哨、措辞更迎合评委家族）；而评测时又由同家族的 judge 给出高分"确认"了进步。训练和评测就这样互相强化了彼此的盲点。这意味着 3.4 的偏差表不只是"评测方法论问题"：**一旦 judge 接入训练回路，它的偏差直接决定模型学到什么**。这也是业界持续投入 judge 校准研究（多裁判组合、风格控制、长度控制）的深层动机。
...
- **锚点模型选择研究**（2026）：针对 Arena-Hard 这类需要固定"参照回答"进行成对比较的评测方式，学界通过对超过 85 万条成对比较数据的分析发现，选用中等水平的模型作为评测锚点，其打分结果与人类偏好的相关性反而高于用顶尖模型或弱模型做锚点——这是一个反直觉但很有工程指导意义的发现：**设计评测系统时，"用最强的模型做所有事"未必是最优解**，评审角色和被评角色对模型能力的要求可能是不同的（"锚点模型"的角色、两种失效模式与统计原理的详解见**附录 A.7**）。
...
| **Anthropic / OpenAI / Google** | 主流厂商的模型发布报告普遍在 Humanity's Last Exam（将在 Week 13 详细讨论）、ARC-AGI-2/3、SWE-bench 系列上同时汇报分数，反映"知识饱和基准+抗饱和高难度基准+真实任务基准"三线并行汇报正成为行业惯例 |
...
- [ ] 理解 ARC-AGI 系列"抗记忆、抗刷题"的设计哲学与其他基准的本质区别，能说出 ARC-AGI → ARC-AGI-2 → ARC-AGI-3 三代演进（静态谜题 → 抗暴力枚举 → 交互式环境）及 RHAE 效率评分的设计意图
...
- [ ] 能说出 LLM-as-Judge 在训练侧的四类角色（RLAIF 偏好信号、数据质量筛选、自蒸馏选优、RL 在线评委），以及训练与评测共用 judge 的自指闭环风险
...
### A.5 位置偏差详解：概念、成因与最小化验证实验

> 关联正文：**3.4 节 → 位置偏差**

**什么是位置偏差**

LLM-as-Judge 的典型用法是成对比较：把两个回答并排交给裁判，请它判断哪个更好——
...
**位置偏差（Position Bias）**指：裁判的判决会系统性地偏袒占据某个位置的回答（多数裁判偏袒排在前面的回答），与回答的实际内容质量无关。它不是"裁判按顺序依次评分、被前者干扰"，而是**恒定的位置性偏袒**——换一对全新内容，它仍偏心同一个位置；把同一对内容顺序对调，它的判决可能跟着翻转。

**如何判定一个裁判是否存在位置偏差**：对同一对回答，正反顺序各评一次——

| 输入顺序 | 理想裁判（无偏差） | 存在位置偏差的裁判 |
|---------|------------------|------------------|
| `[A 在前, B 在后]` | 判 A 更好 | 判"位置 1"（即 A）更好 |
| `[B 在前, A 在后]` | 仍判 A 更好（位置标签变了，但指向同一实体） | 仍判"位置 1"（即 B）更好 |

两次判决指向同一实体（只是位置标签互换），说明裁判在比较内容；两次都判"排在前面的赢"，说明判决被位置信号主导。

**成因直觉**：自回归生成中，先出现的回答为后续 token 的生成设定了参考框架（先读到的内容塑造理解后续内容的语境），产生"先入为主"的锁；其次训练语料（尤其是评测类文本）本身存在"先列更优选项"的分布习惯，模型从中继承了"靠前的更可能正确"的先验。需要注意，偏差方向（偏好靠前还是靠后）因裁判模型而异，只能通过实测确定。

**为什么必须显式消偏**：位置偏差是有固定方向的**系统性误差**，而非随机噪声——随机噪声可靠"多跑题自然平均"削弱，系统性误差不会随样本量增加而自动消失，它会一致性地把排名拉向同一方向。因此必须靠"正向+反向各评一次、聚合取平均"在实验设计层面显式对冲，这也是 MT-Bench 论文系统报告该现象后双向评测成为成对比较评测标配的原因。

**最小化验证实验**

有了上面的判定逻辑，验证实验本身非常轻量——几乎不需要额外基础设施、任何人都能复现：
...
### A.6 AIME"时间切片"设计的三个代价：跨年可比性、统计噪声与饱和加速

> 关联正文：**2.2 节 → "饱和与升级"中的 AIME**

**问题**：AIME 靠"每年出新题"天然获得抗污染能力——题目在模型训练 cutoff 之后才发布，不可能被背进训练数据，这是它作为推理模型基准的核心优势。但这种"时间切片防污染"设计并非零成本，代价体现在以下三个方面。

**代价一：纵向可比性失效，但横向可比性保留**

题池跟着年份轮转意味着 AIME 2024 的分数与 AIME 2025 的分数测的是**两套不同的题**，二者之间不存在共同参照系，严格的跨年份（纵向）比较因此失效。但值得强调的是，**横向比较并未失效**——同一年内的各家前沿模型测的是同一个试题集，例如各家报告的 AIME 2025 分数仍然构成严格有效的横截面排名。

换句话说，"时间切片"把评测比较的有效窗口压缩到了大约一年这个量级。一个务实的解读是：在模型能力以季度为单位演进的当下，跨年度的精确纵向对比本就已丧失大部分实践意义——"时间切片"只是把这一事实显式化了，真正受损的不是评测的严谨性，而是"跨年代际对比"这一叙事习惯。

**代价二：不同年份难度分布波动——比"题不一样"更隐蔽的问题**

即使承认题不同，还有一层更微妙的干扰：**不同年份的 AIME 难度分布本身就有波动**。出题团队的风格、当年题目的结构（计算密集型 vs 技巧密集型）都会带来偏倚。这意味着"AIME 2024 考 90% vs AIME 2025 考 85%"不能直接读成"能力退步 5%"——这 5 个百分点里有多少来自难度变化，无法从分数本身分离。

正因为这一点，像 MathArena 这类滚动评测框架在设计上刻意保证一点：**用同一批最新竞赛题同时评测所有新发布的模型**，强制保持分母一致，而不是允许各家自行挑选有利的年份报告。

**代价三：题量太小，单次评测统计噪声大**

AIME 每年只有 30 题（AIME I、AIME II 两卷各 15 题）。在 30 个样本上，一道题的运气波动就是约 3.3 个百分点的分数差，三道题的偏差即可造成 10 个百分点的表观差异。因此前沿模型报告 AIME 分数时几乎普遍采用 **avg@k**（对每题独立采样 $k$ 次取平均正确率，常见 $k$ = 16、64）而非单次 pass@1。这与 Part 2.1 代码基准中 pass@k 面临的统计问题一脉相承——小题量基准天然对单次评测敏感，多次采样平均是必要而非可选的修正。

**副产品：AIME 正在快速饱和**

推理模型的演进速度大幅压缩了 AIME 的生命周期：自 2024 年底被大规模采用为推理模型标准基准以来，第一梯队模型在 2025 年下半年已进入 95%+ 区间，区分度正在快速消失。这延续了正文 2.2 节的演进链逻辑：

```
GSM8K（小学应用题，2021）→ MATH（竞赛题，2021）→ AIME（奥赛入门，历年赛事）
    → FrontierMath（研究级，2024）→ HMMT / IMO 级（2025+）
```

一个基准保持"有区分度的前沿基准"定位通常只有两三年。这并非 AIME 独有的命运，而是评测体系的根本特征：**只要模型能力增速快于基准难度储备，基准就必须不断向下一代迭代**——Part 1（MMLU 系列）和 Part 2（代码/数学基准）反复出现同一模式，原因均在于此。

### A.7 锚点模型的"倒 U 形"：为什么中等水平模型是最好的参照物

> 关联正文：**4.1 节 → 锚点模型选择研究；3.4 节末段**

**问题**：在 Arena-Hard 这类依据"参照回答"进行成对比较的评测中，生成参照回答的模型（锚点模型，anchor/baseline）该选多强？直觉答案是"越强越严格"，但研究发现锚点模型能力与评测结果同人类偏好的相关性呈"倒 U 形"——**中等水平模型反而最优**。要理解这个反直觉结论，需要先厘清锚点模型扮演的角色。

**锚点模型的角色：不是裁判，是参照物**

回顾 Arena-Hard 的打分机制：对每道难题，被测模型 A 生成一份回答，固定的锚点模型 B 生成一份参照回答，两者送交 LLM 裁判进行成对比较；**模型 A 的最终得分 = 它在题库上赢过锚点模型的胜率**。排行榜上所有分数都是"相对同一参照物的胜率"——借助攀岩馆的类比：所有选手爬的都是同一条参照线，报告的成绩是"比参照线多爬了几米"。

注意这里存在两个不同的角色：**裁判（judge）**负责读两个回答并投票；**锚点（anchor）**只负责产出参照回答。后文的结论只针对锚点的选择，不涉及裁判的选择。

**失效模式一：锚点是顶尖模型 → "地板效应"**

所有被测模型都输给它，胜率被压缩在极低区间（如 5%–15%）。排名问题退化为"比谁输得不那么惨"，顶尖被测模型之间的细微差异无法传导进分数。此外，judge 面对"锚点明显更好"的比较时，判决几乎完全被锚点的绝对质量决定，比较信号携带关于被测模型的信息极少。

**失效模式二：锚点是很弱的模型 → "天花板效应"**

所有被测模型都碾压它，胜率被压缩在极高区间（如 90%–99%），区分度同样丢失。更糟的是，"赢一个明显更弱的回答"对 judge 是个粗糙任务——内容、结构、完整性全方位碾压时，judge 无需做任何精细的质量权衡就能轻松判胜，判决结果也就不携带多少有辨识度的信息。

**为什么中等水平锚点最优**

1. **区分度最大化**：中等水平锚点大致位于被测模型能力分布的中间位置，各模型的胜率自然散布在较宽区间（如 20%–80%），细微的能力差距能映射为可测量的胜率差。
2. **比较任务变得更精细**：大量"两个质量相近回答"的比较，**迫使 judge 进行细粒度判断**（内容是否正确、是否答到点上），而不是被长度、格式等表面差异主导——这类判决更贴近人类评审员的实际判断模式。
3. **统计学原理**：Bradley-Terry 这类成对比较模型中，单次比较对参数估计贡献的信息量在双方实力接近（胜率接近 50%）时达到峰值；锚点是所有模型的"公共对手"，公共对手越强或越弱，所有配对的信息量都随之下降。

生活类比：想通过"与同一个公共对手下棋"为一群棋手排名时，选棋王（大家都输，只比输多惨）或选新手（大家都赢，只比赢多快）都不是好参照；选一个水平居中的对手，每盘胜负对人群能力分布的刻画才最细腻。

**两个易混淆点的澄清**

- 该结论与"裁判应该用最强的模型"并不矛盾：**裁判是"打分的人"，判断力越强越好；锚点是"被比的靶子"，作用是可区分化，强度居中信息量最大**。这正是"评审角色与被评角色对模型能力的要求不同"的具体含义。
- 该发现的工程推论值得注意：Arena-Hard 长期使用的锚点是 GPT-4-0314——它在 2024 年属强模型，随着被测模型整体水平提升，其相对位置自然滑向"中等"，无意中恰好满足倒 U 形的研究结论；但如果题库和锚点长期不更新（被测模型整体把它们远远地甩在后面），锚点会滑向失效模式二（天花板效应），这是静态参照物设计的内在老化问题。
